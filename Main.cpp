...
enum class WeaponType { LASER, BULLET,EXPLOSIVE, COUNT };
class Projectile {
public:
	bool fragmentOnHit;
	Projectile(Vector2 pos, Vector2 vel, int dmg, WeaponType wt, bool fragment = false)
	{
		transform.position = pos;
		physics.velocity = vel;
		baseDamage = dmg;
		type = wt;
		fragmentOnHit = fragment;
	}
	bool Update(float dt) {
		transform.position = Vector2Add(transform.position, Vector2Scale(physics.velocity, dt));
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			...
			static Texture2D rocketTexture = LoadTexture("rocket.png"); // Make sure rocket.png exists
			float scale = 0.01f; // Adjust as needed
			Vector2 pos = {
				transform.position.x - (rocketTexture.width * scale) * 0.5f,
				transform.position.y - (rocketTexture.height * scale) * 0.5f
			};
		
			DrawTextureEx(rocketTexture, pos, 0.0f, scale, WHITE);
		}
	}
	Vector2 GetPosition() const {
		return transform.position;
	...
	float speed)
{
	...
	if (wt == WeaponType::EXPLOSIVE)
		return Projectile(pos,vel,15,wt,true);
	else {
	...
	void Run() {
		...
		while (!WindowShouldClose()) {
			...
			for (auto pit = projectiles.begin(); pit != projectiles.end();) {
				...
				for (auto ait = asteroids.begin(); ait != asteroids.end(); ++ait) {
					...
					if (dist < (*pit).GetRadius() + (*ait)->GetRadius()) {
						...
						if (pit->fragmentOnHit) {
							const int fragmentCount = 5;
							float angleStart = GetRandomValue(0, 360);
							float angleStep = 360.0f / fragmentCount;
							float fragmentSpeed = 200.0f;

							for (int i = 0; i < fragmentCount; ++i) {
								float angleDeg = angleStart + i * angleStep;
								float angleRad = angleDeg * DEG2RAD;
								Vector2 fragVel = { cosf(angleRad) * fragmentSpeed, sinf(angleRad) * fragmentSpeed };
								projectiles.emplace_back(pit->GetPosition(), fragVel, 5, WeaponType::BULLET);
							}
						}
			...
			// Render everything
			{
				...
				const char* weaponName = (currentWeapon == WeaponType::LASER) ? "LASER" :
